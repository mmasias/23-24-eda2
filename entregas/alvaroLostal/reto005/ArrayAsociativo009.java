package entregas.alvaroLostal.reto005;
...
public class ArrayAsociativo009 {
    ...
    private static void imprimirStatus(int[] personaje) {
        ...
        System.out.println(" / v: SKIN[" + modoVisualizacion + "] / e: MODO AUTOMATICO");
        imprimirLinea();
    }
    ...
    static int[][] inicializarMaze(String[] mapa) {
        ...
        for (int fila = 0; fila < mapa.length; fila++) {
            for (int columna = 0; columna < mapa[0].length(); columna++) {
                if (mapa[fila].charAt(columna) == ' ' || mapa[fila].charAt(columna) == '-'
                        || mapa[fila].charAt(columna) == '|' || mapa[fila].charAt(columna) == '~') {
                    maze[fila][columna] = WALL;
                } else {
                    maze[fila][columna] = FREE;
                }
            }
        }
        return maze;
