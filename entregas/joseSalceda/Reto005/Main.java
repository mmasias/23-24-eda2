package entregas.joseSalceda.Reto005;
...
public class Main {
    ...
    static List<int[]> findPath(int[] start, int[] target, String[] map) {
        int filas = map.length;
        int columnas = map[0].length();
    
        // Check if the target position is within the valid boundaries of the map
        if (target[FILA] < minFila || target[FILA] > maxFila || target[COLUMNA] < minColumna || target[COLUMNA] > maxColumna) {
            System.out.println("Target position is out of bounds. Returning empty path.");
        ...
        if (findPathHelper(start, target, map, visited, path)) {
            return path;
        } else {
    ...
    static boolean findPathHelper(int[] current, int[] target, String[] map, boolean[][] visited, List<int[]> path) {
        if (current[FILA] == target[FILA] && current[COLUMNA] == target[COLUMNA] && tipoTerreno(map[target[FILA]].charAt(target[COLUMNA])).equals("0")) {
            path.add(current);
            return true;
        }
        ...
        for (int[] direction : MOVIMIENTO) {
            ...
            if (newFila >= minFila && newFila <= maxFila && newColumna >= minColumna && newColumna <= maxColumna) {
                if (!visited[newFila][newColumna] && tipoTerreno(map[newFila].charAt(newColumna)).equals("0")) {
                    int[] newPosition = { newFila, newColumna };
                    if (findPathHelper(newPosition, target, map, visited, path)) {
                        path.add(0, current);
                        return true;
                    }
                }
    ...
    static void verAccion(int[] elPersonaje, String[] elMundo) {
        ...
        switch (capturarMovimiento()) {
            ...
            case BUSCAR_RUTA:
                ...
                int[] targetPosition = { 40, 42 };
                List<int[]> path = findPath(startPosition, targetPosition, elMundo);
                if (path != null) {
