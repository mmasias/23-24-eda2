package entregas.lavinDaniel.reto005;
...
public class Laberinto {
        ...
        static final int BUSCAR_RUTA = 6;
    ...
    static void mover(int[] unPersonaje, int direccion, String[] mapa, boolean seguirRuta) {
        if (seguirRuta) {
    ...
    static void verAccion(int[] elPersonaje, String[] elMundo) {
        int accion = capturarMovimiento();
        switch (accion) {
            case ARRIBA:
                mover(elPersonaje, ARRIBA, elMundo, false);
                break;
            case ABAJO:
                mover(elPersonaje, ABAJO, elMundo, false);
                break;
            case IZQUIERDA:
                mover(elPersonaje, IZQUIERDA, elMundo, false);
                break;
            case DERECHA:
                mover(elPersonaje, DERECHA, elMundo, false);
                break;
            case SALIR:
                jugando = !jugando;
                break;
            case CAMBIA_VISUALIZACION:
                ...
                break;
            case BUSCAR_RUTA:
                buscarRuta(elPersonaje, elMundo, 70, 39); // Buscar ruta hacia X=70, Y=39
                break;
            case NADA:
                break;
            default:
        ...
        static void buscarRuta(int[] elPersonaje, String[] elMundo, int objetivoX, int objetivoY) {
          boolean[][] visitado = new boolean[elMundo.length][elMundo[0].length()];
          if (buscarRutaRecursiva(elPersonaje[FILA], elPersonaje[COLUMNA], objetivoX, objetivoY, elMundo, visitado)) {
            System.out.println("Ruta encontrada. Siguiendo la ruta...");
            seguirRuta(elPersonaje, objetivoX, objetivoY, elMundo, visitado);
          } else {
            System.out.println("No se encontró una ruta válida.");
          }
    }

       static boolean buscarRutaRecursiva(int fila, int columna, int objetivoX, int objetivoY,
                                       String[] mundo, boolean[][] visitado) {
          if (fila < 0 || fila >= mundo.length || columna < 0 || columna >= mundo[0].length() ||
                mundo[fila].charAt(columna) == '#' || visitado[fila][columna]) {
            return false;
          }

          visitado[fila][columna] = true;

          if (fila == objetivoX && columna == objetivoY) {
            return true;
          }

          for (int[] dir : MOVIMIENTO) {
            int nuevaFila = fila + dir[FILA];
            int nuevaColumna = columna + dir[COLUMNA];
            if (buscarRutaRecursiva(nuevaFila, nuevaColumna, objetivoX, objetivoY, mundo, visitado)) {
                return true;
            }
          }
    ...
    static void seguirRuta(int[] elPersonaje, int objetivoX, int objetivoY,
                           String[] elMundo, boolean[][] visitado) {
        int filaActual = elPersonaje[FILA];
        int columnaActual = elPersonaje[COLUMNA];

        if (filaActual == objetivoX && columnaActual == objetivoY) {
            System.out.println("¡Objetivo alcanzado!");
            return;
        }

        for (int[] dir : MOVIMIENTO) {
            int nuevaFila = filaActual + dir[FILA];
            int nuevaColumna = columnaActual + dir[COLUMNA];
            if (nuevaFila >= 0 && nuevaFila < elMundo.length &&
                    nuevaColumna >= 0 && nuevaColumna < elMundo[0].length() &&
                    !visitado[nuevaFila][nuevaColumna] && elMundo[nuevaFila].charAt(nuevaColumna) != '#') {
