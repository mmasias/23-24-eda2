...
public class ArrayAsociativo009 {
    ...
    public static void main(String[] args) {
        ...
        if (auto) {
            System.out.println("Empezando automáticamente");
            matriz = startMaze(castilloLB);
            if (moveMaze(matriz, elPersonaje, DERECHA, castilloLB)) {
                imprimirMundo(castilloLB, elPersonaje);
            }
            else {
    ...
    static int[][] startMaze (String[] maze) {
        int[][] matriz = new int[maze.length][maze[0].length()];
        for (int i = 0; i < maze.length; i++) {
            for (int j = 0; j < maze.length; j++) {
                if (maze[i].charAt(j) == '-' || maze[i].charAt(j) == '~' || 
                        maze[i].charAt(j) == ' ' || maze[i].charAt(j) == '|') {
                    matriz[i][j] = WALL;
                }
                else {
                    matriz[i][j] = FREE;
    ...
    static boolean moveMaze(int[][ ] maze, int [] personaje, int direccion, String[] mapa){
        ...
        int[] direcciones = {ARRIBA, ABAJO, IZQUIERDA, DERECHA};
        for (int dir : direcciones) {
            if (moveMaze(maze, personaje, dir, mapa)) {
                return true;
            }
        }
        maze[x][y] = VISITED;
