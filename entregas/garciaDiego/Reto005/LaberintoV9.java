package entregas.garciaDiego.Reto005;

import java.util.*;

public class LaberintoV9 {
    ...
    public static void main(String[] args) {
        ...
        if (automatico) {
            ...
            maze = initMaze(castilloLB);
            if (completarAutomatico(maze, elPersonaje, DERECHA, castilloLB)) {
                imprimirMundo(castilloLB, elPersonaje);
            } else {
    ...
    static int[][] initMaze(String[] mundo) {
        int[][] mapa = new int[mundo.length][mundo[0].length()];
        for (int i = 0; i < mundo.length; i++) {
            for (int j = 0; j < mundo.length; j++) {
                mapa[i][j] = Integer.parseInt(tipoTerreno(mundo[i].charAt(j)));
            }
        }
        return mapa;
    }

    static boolean completarAutomatico(int[][] maze, int[] personaje, int direccion, String[] mapa) {
        ...
        if (x < 0 || x >= maze.length || y < 0 || y >= maze[0].length) {
            System.out.println("Intentando moverse a: (" + x + ", " + y + "): Fuera de los límites del laberinto.");
            return false;
        }
        if (maze[x][y] != FREE) {
            System.out.println("Intentando moverse a: (" + x + ", " + y + "): El camino esta bloqueado.");
            return false;
        }
        if (x == 14 && y == 36) {
            maze[x][y] = PATH;
            System.out.println("Intentando moverse a: (" + x + ", " + y + "): HAS LLEGADO A LA SALIDA.");
            return true;
        }
        maze[x][y] = PATH;
        ...
        if (completarAutomatico(maze, personaje, DERECHA, mapa)) {
            return true;
        }
        if (completarAutomatico(maze, personaje, IZQUIERDA, mapa)) {
            return true;
        }
        if (completarAutomatico(maze, personaje, ARRIBA, mapa)) {
            return true;
        }
        if (completarAutomatico(maze, personaje, ABAJO, mapa)) {
            return true;
        }
        maze[x][y] = VISITED;
        imprimirMundo(mapa, personaje);
