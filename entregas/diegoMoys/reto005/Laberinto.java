package entregas.diegoMoys.reto005;
...
public class Laberinto {
    ...
    static boolean buscandoSalida = false;
    ...
    public static void main(String[] args) {
        ...
        } while (jugando && !buscandoSalida);
        if (buscandoSalida)

        {
            laberinto = inicializarLaberinto(castilloLB);
            System.out.println("Modo buscar salida automáticamente activado");
            if (movimientoAuto(laberinto, elPersonaje, ultimoMovimiento, castilloLB)) {
                imprimirMundo(castilloLB, elPersonaje);
            } else {
                System.out.println("No se puede alcanzar la salida");
            }
        }
    ...
    static boolean movimientoAuto(int[][] laberinto, int[] personaje, int direccion, String[] mapa) {
        actualizarTiempo();
        ...
        if (laberinto[x][y] != libre) {
            System.out.println("No es un camino libre.");
            return false;
        }
        if (x == 40 && y == 42) {
        ...
        System.out.println("Intentando moverse a: (" + y + ", " + x + "): Marcando como camino.");
        try {
            Thread.sleep(100);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        if (movimientoAuto(laberinto, personaje, DERECHA, mapa)) {
            return true;
        }
        if (movimientoAuto(laberinto, personaje, ABAJO, mapa)) {
            return true;
        }
        if (movimientoAuto(laberinto, personaje, IZQUIERDA, mapa)) {
            return true;
        }
        if (movimientoAuto(laberinto, personaje, ARRIBA, mapa)) {
            return true;
        }
        laberinto[x][y] = visitado;
        System.out.println("Intentando moverse a: (" + x + ", " + y + "): No es posible pasar");
        imprimirMundo(mapa, personaje);
        return false;
    }
