...
public class ArrayAsociativo009 {
    ...
    */
    ...
    static List<int[]> buscarutas(int[] start, int[] destino, String[] map) {
        int filas = map.length;
        int columnas = map[0].length();
    
        // Check if the destino position is within the valid boundaries of the map
        if (destino[FILA] < minFila || destino[FILA] > maxFila || destino[COLUMNA] < minColumna || destino[COLUMNA] > maxColumna) {
            System.out.println("Adonde quiero ir está fuera de los límites");
        ...
        if (BuscaRutasHelper(start, destino, map, visitado, ruta)) {
            return ruta;
        } else {
            System.out.println("Es imposible llegar ahí");
            return new ArrayList<>();
        }
    }
    
    static boolean BuscaRutasHelper(int[] current, int[] destino, String[] map, boolean[][] visitado, List<int[]> ruta) {
        if (current[FILA] == destino[FILA] && current[COLUMNA] == destino[COLUMNA] && tipoTerreno(map[destino[FILA]].charAt(destino[COLUMNA])).equals("0")) {
            ruta.add(current);
            return true;
        }
    
        visitado[current[FILA]][current[COLUMNA]] = true;
    
        for (int[] direction : MOVIMIENTO) {
            int newFila = current[FILA] + direction[FILA];
            int newColumna = current[COLUMNA] + direction[COLUMNA];
    
            // Check if the new position is within the valid boundaries of the map
            if (newFila >= minFila && newFila <= maxFila && newColumna >= minColumna && newColumna <= maxColumna) {
                if (!visitado[newFila][newColumna] && tipoTerreno(map[newFila].charAt(newColumna)).equals("0")) {
                    int[] newPosition = { newFila, newColumna };
                    if (BuscaRutasHelper(newPosition, destino, map, visitado, ruta)) {
                        ruta.add(0, current);
                        return true;
                    }
                }
    ...
    static void verAccion(int[] elPersonaje, String[] elMundo) {
        ...
        switch (capturarMovimiento()) {
            ...
            case BUSCAR_RUTA:
                ...
                int[] destinoPosition = { 70, 39 };
                List<int[]> ruta = buscarutas(startPosition, destinoPosition, elMundo);
                if (ruta != null) {
                    for (int[] position : ruta) {
