...
public class ArrayAsociativo009 {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        int[] lastPosition = { 31, 33 };

        inicializarMundo(dosCastillos);

        do {
            actualizarTiempo();
            imprimirMundo(dosCastillos, elPersonaje);
            verAccion(elPersonaje, dosCastillos);
        } while (jugando);
    }
    ...
    static void verAccion(int[] elPersonaje, String[] elMundo) throws InterruptedException {
        ...
        switch (capturarMovimiento()) {
            ...
            case SALIR:
                ...
                break;
            case AUTO:
                resolverAuto(elPersonaje, elMundo);
                break;
    ...
    static int[][] convertirMundo(String[] elMundo){
        int[][] maze = new int[elMundo.length][elMundo[0].length()];
        for (int i = 0; i < elMundo.length; i++) {
            for (int j = 0; j < elMundo.length; j++) {
                    if(elMundo[i].charAt(j)==' '  || elMundo[i].charAt(j)=='-'|| elMundo[i].charAt(j)=='|'|| elMundo[i].charAt(j)=='~'){
                        maze[i][j]=WALL;
