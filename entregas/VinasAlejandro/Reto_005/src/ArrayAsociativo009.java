...
public class ArrayAsociativo009 {
    ...
    static final int RESUELVE = 6;
    static final int RESUELVEYMUEVE = 7;
    ...
    public static void main(String[] args) {
        ...
        do {
            ...
            if(resuelto)
                if (moverP)
                {
                    if (imprimirMoviendo(castilloLB, elPersonaje))
                    {
                    ...
                    imprimirMundoRes(castilloLB, elPersonaje);
            else
    ...
    static String mapearNormal(char elemento) {
        ...
        tiles.put("PATH", BLUE_BACKGROUND + "  U  " + RESET);
        return tiles.get("" + elemento);
    }
    ...
    static void verAccion(int[] elPersonaje, String[] elMundo) {        
        switch (capturarMovimiento()) {
            ...
            case CAMBIA_VISUALIZACION:
                ...
                break;
            case RESUELVE:
                solucion=resuelve(elPersonaje,elMundo);
                if(solucion!=null)
                    resuelto=true;
                break;
            case RESUELVEYMUEVE:
    ...
    private static int[][] resuelve(int[] elPersonaje, String[] elMundo) {
        int[][] maze=new int[elMundo.length][elMundo[0].length()];
        int i=0;
        for (String cadena : elMundo) {
            for (int j=0;j<cadena.length();j++)
            {
                maze[i][j]=Integer.parseInt(tipoTerreno(cadena.charAt(j)));
            }
            i++;
        }
        
        if (solveMaze(maze, elPersonaje[0], elPersonaje[1])) {
            System.out.println("Ruta de escape facilitada");
            resuelto=true;
            return maze;
        } else {
    ...
    static void imprimirMundoRes(String[] castillo, int[] personaje) {

        String elemento;
        String uno=String.valueOf(personaje[FILA]) + String.valueOf(personaje[COLUMNA])+" ";
        limpiarPantalla();
        imprimirElCielo();
        for (int fila = personaje[FILA] - viewPort; fila <= personaje[FILA] + viewPort; fila++) {
            for (int columna = personaje[COLUMNA] - viewPort; columna <= personaje[COLUMNA] + viewPort; columna++) {
                ...
                if (solucion[fila][columna]==2 && uno.compareTo(String.valueOf(fila) + String.valueOf(columna)+" ")!=0)
                {
                    elemento="PATH";
                }
                else{
    ...
    static boolean imprimirMoviendo(String[] castillo, int[] personaje)
    {
        int[] posOldPersonaje=new int[2];
        posOldPersonaje[0]=0;
        posOldPersonaje[1]=0;
        int[] persoAnt=new int[2];
        persoAnt[0]=0;
        persoAnt[1]=0;

        int[] personajenew=new int[2];
        personajenew[0]=0;
        personajenew[1]=0;
        imprimirMundoRes(castillo, personaje);
        
        while (!(personajenew[0]==llegada[0] && personajenew[1]==llegada[1]))
        {      
            personajenew=posNuevaPersonaje(castillo,personaje);
            posOldPersonaje=personaje; 
            imprimirMundoResPersonajeMove(castillo, personajenew,posOldPersonaje); 
    ...
    static int[] posNuevaPersonaje(String[] castillo, int[] personaje)
    {
        int [] posicion=new int[2];     
        int fila=personaje[FILA];
        int col=personaje[COLUMNA];    
    

        if (solucion[fila-1][col]==2)
        {
            posicion[0]=fila-1;
            posicion[1]=col;
        }
        else if (solucion[fila+1][col]==2)
        {
            posicion[0]=fila+1;
            posicion[1]=col;
        }
        else if (solucion[fila][col-1]==2)
        {
            posicion[0]=fila;
            posicion[1]=col-1;
        }
    ...
    static void imprimirMundoResPersonajeMove(String[] castillo, int[] personaje,int[] posOldPersonaje) {
        ...
        for (int fila = personaje[FILA] - viewPort; fila <= personaje[FILA] + viewPort; fila++) {
            for (int columna = personaje[COLUMNA] - viewPort; columna <= personaje[COLUMNA] + viewPort; columna++) {
                ...
                if (solucion[fila][columna]==2 && uno.compareTo(String.valueOf(fila) + String.valueOf(columna)+" ")!=0)
                {
                    solucion[posOldPersonaje[0]][posOldPersonaje[1]]=0;                   
                    elemento="PATH";                    
                }
                else{
    ...
    public static boolean solveMaze(int[][] maze, int x, int y) {
        ...
        if (x == llegada[0] && y == llegada[1]) { //destino
            maze[x][y] = PATH;
            return true;
        }
        maze[x][y] = PATH;
